...
package body Implementation_For_Architecture
  is pragma Source_File_Name("neo-processor-architecture.adb");
  ----------------
  -- Exceptions --
  ----------------
  ...
  ----------------
    procedure Initialize
      ...
      begin
        if Get_Bit_Size < 32 then
          raise CPUID_Is_Not_Supported;
        elsif Get_Bit_Size = 32 then
          Asm( -- Check for cpuid
            Volatile => True,
            Template =>
              ------------------------------------------
...
if Get_Bit_Size >= 64 then Put_Line("Put_Stack is disabled due to an assembly block bug on 64 bit computers."); raise Disable_Put_Stack; end if;
        Asm(
          Volatile => True,
          Inputs =>(
