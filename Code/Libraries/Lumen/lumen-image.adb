...
package body Lumen.Image is
  ...
  function To_Array_Record_Pixel(
    Descriptor_R : in Descriptor)
    return Array_Record_Pixel
    is
    Pixels : Array_Record_Pixel(1..Descriptor_R.Width, 1..Descriptor_R.Height);
    begin
      if Descriptor_R.Values = null then
        raise Invalid_Format;
      end if;
      for I in Descriptor_R.Values'range loop
        for J in Descriptor_R.Values(I)'range loop
          Pixels(I, J) :=(
            Alpha  => Descriptor_R.Values(I, J).A,
            Colors =>(
              Red   => Descriptor_R.Values(I, J).R,
              Green => Descriptor_R.Values(I, J).G,
              Blue  => Descriptor_R.Values(I, J).B),
        end loop;
      end loop;
      return Pixels;
    end To_Array_Record_Pixel
end Lumen.Image;
